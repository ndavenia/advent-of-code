...
class Computer {
    ...
    compute(input) {
        ...
        for (; ;) {
            ...
            const getIndex = pos => {
                const iMode = ~~(parameter / Math.pow(10,pos + 1)) % 10;
                switch(iMode) {
                    case 0:
                        return this.program[this.i + pos];
                    case 1:
                        return this.i + pos;
                    case 2:
                        return this.program[this.i + pos] + this.relativeBase;
                }
            };
            const index1 = getIndex(1);
            const index2 = getIndex(2);
            const index3 = getIndex(3);
            ...
            if (instruction === 1) {
                ...
                this.program[index3] = this.program[index1] === this.program[index2] ? 1 : 0;
                this.i += 4;
            } else if (instruction === 9) { // adjusts the relative base
                this.relativeBase += this.program[index1];
                this.i += 2;
            }
            else {
...
console.log("part 1 : ", comp.compute([1]));

var comp2 = new Computer();
console.log("part 2 : ", comp2.compute([2]));
